...
from app.domain.catalog import Money, Sku
from app.domain.catalog import Product as CatalogProduct
...
def _sku(
    ...
    """保存价格与库存，使其能进入当前 Product 的检索文本。"""

    return _SeedSku(sku_id, spec, major, currency, stock)
...
def Product(
    ...
    **unused: Any,
    ...
    """把旧版丰富商品参数无损投影到项目当前的通用 Product。"""

    del unused
    attributes = {
        "产地": origin_country,
        "可配送国家地区": ", ".join(ships_to),
        "SKU价格库存": "；".join(
            f"{item.spec} {item.major:g} {item.currency} 库存{item.stock}"
            for item in skus
        ),
    }
    attributes.update(
        {f"亮点-{item.name}": item.value for item in highlights}
    )
    return CatalogProduct(
        ...
        ships_to=tuple(ships_to),
        skus=tuple(
            Sku(
                sku_id=item.sku_id,
                spec=item.spec,
                price=Money.from_major_units(item.major, item.currency),
                stock=item.stock,
            )
            for item in skus
        ),
        primary_sku_id=skus[0].sku_id if skus else None,
