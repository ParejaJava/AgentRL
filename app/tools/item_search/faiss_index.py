...
from .ports import ItemVectorIndex
from .schemas import FloatVector, Product, RecallHit
...
class FaissItemIndex:
    ...
    def load(cls, directory: Path) -> "FaissItemIndex":
        ...
        if len(products) != manifest.item_count:
            raise ValueError("index manifest item_count does not match product metadata")
    ...
    def get(self, index_id: str) -> FaissItemIndex:
        with self._lock:
            ...
            if manifest.embedding_model != self._expected_embedding_model:
                raise ValueError("index embedding model does not match the online encoder")
